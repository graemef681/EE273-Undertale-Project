...
float Travel::pathfinding_algorithm(Destination* start_dest, Destination* end_dest, std::list<Destination>* Top)
{
		...
		while (shortest.getName() != end_dest->getName())
		{
			...
			cout << "Current node is: " << curNode->getName();
			...
			while (NIt != end)
			{
				...
				{ //if the currentDistance is shorter than the previous
					shortest = *NIt;
					cout << "Number of neighbours for shortest is: " << shortest.getNoNeighbours();
			...
			if (!visitedBefore)
			{
				...
				curNode = &shortest;
				cout << "The current node is now:" << shortest.getName() << endl;
				//visitedBefore = false;
			}
		}
